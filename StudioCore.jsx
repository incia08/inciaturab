...
function InvoiceForm({ invoices, setInvoices, showToast }) {
  ...
  const saveInvoice = async () => {
    ...
    const inv = { ...form, id: Date.now().toString(), total: subtotal, discount: discountAmt, createdAt: new Date().toISOString() };
    ...
    <div>
      ...
      {inv.id && !onSave && (
        <button className="btn btn-outline" onClick={() => {
          const link = `${window.location.origin}/invoice?id=${inv.id}`;
          navigator.clipboard?.writeText(link) || (window.prompt("Copy link:", link));
          alert("Invoice link copied! Send to client on WhatsApp 🌸\n\n" + link);
...
export default function StudioCore({ supabase }) {
